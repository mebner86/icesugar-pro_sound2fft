...
consecutive left-channel samples (top 16 bits of each 24-bit I2S word) into
on-chip BRAM.  A second command streams those samples back to the host as raw
big-endian 16-bit PCM over the iCELink USB-CDC virtual COM port.
...
| Sample width | 16 bits (top 16 of 24-bit I2S word) |
| Block size | 4096 samples |
| Record duration | ~84 ms |
| UART baud rate | 115200 8N1 |
| Dump size | 8192 bytes (4096 × 2) |
| Dump time | ~711 ms |
...
| `'D'` (0x44) | CMD_DUMP | Stream 4096 samples as 8192 bytes (big-endian 16-bit) |
...
i2s_clkgen (CLK_DIV=4) + i2s_rx (DATA_BITS=24)
  ...
  │  truncate → rx_left_data[23:8]
  ▼
sample_ram [0:4095] reg [15:0]   (inferred ECP5 EBR)
  ▲                   │
...
3. Sends `'D'` and receives 8 bytes
4. Verifies each sample matches the expected top-16 bits
...
8192-byte payload, removes the microphone DC offset, then shows a two-panel
...
with serial.Serial('/dev/ttyACM0', 115200, timeout=2) as s:
    ...
    s.write(b'D')
    raw = s.read(4096 * 2)    # 8192 bytes

samples = struct.unpack('>4096h', raw)  # big-endian signed 16-bit
```
...
dd if=/dev/ttyACM0 bs=8192 count=1 of=capture.pcm

# Convert to WAV (requires sox)
sox -r 48828 -e signed -b 16 -c 1 capture.pcm capture.wav
```
