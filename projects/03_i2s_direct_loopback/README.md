# 03_i2s_direct_loopback

I2S audio direct loopback for the iCESugar-Pro board.

## Description

Reads audio from an SPH0645 MEMS microphone over I2S and forwards it directly to a MAX98357A I2S amplifier — **without any deserialization or reserialization**. The FPGA acts as I2S clock master, generating BCLK and LRCLK, while the microphone's serial data line (DOUT) is wired combinatorially straight to the amplifier's data input (DIN).

This is the simplest possible I2S loopback: one wire from mic to amp, no sample processing at all. Compare to [04_i2s_serdes_loopback](../04_i2s_serdes_loopback), which deserializes into a 24-bit parallel sample and re-serializes.
...
```bash
make build 03   # Build bitstream
make sim 03     # Run simulation
```

Or directly inside the project directory:
...
After running simulation, view the waveforms with GTKWave:

```bash
gtkwave projects/03_i2s_direct_loopback/i2s_direct_loopback_tb.gtkw
```
