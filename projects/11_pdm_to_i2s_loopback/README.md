...
```bash
make build PROJECT=11_pdm_to_i2s_loopback   # Build bitstream
make sim PROJECT=11_pdm_to_i2s_loopback     # Run simulation
```

Or inside the Docker container (`make docker-shell`), run directly:

```bash
cd projects/11_pdm_to_i2s_loopback
...
```bash
gtkwave projects/11_pdm_to_i2s_loopback/pdm_to_i2s_loopback_tb.gtkw
```
...
```bash
make program PROJECT=11_pdm_to_i2s_loopback
```
