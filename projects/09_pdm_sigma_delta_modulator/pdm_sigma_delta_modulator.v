// pdm_sigma_delta_modulator.v — Sigma-delta modulator demo for iCESugar-Pro (ECP5-25F)
//
// Demonstrates the shared rtl/pdm_modulator module in isolation by feeding a
// 64-sample sine ROM through the sigma-delta modulator to the MAX98358 PDM
// amplifier, producing a steady ~763 Hz tone.
//
// Signal flow:
//   ROM (64 × 16-bit sine, init with `initial`) → zero-order hold
//                                               → sigma-delta modulator (1st or 2nd order)
//                                               → 1-bit PDM @ 3.125 MHz
//                                               → MAX98358 PDM amp
//
// Timing:
...
// Running (rst_n = 1): green LED on, tone active.

`default_nettype none

module pdm_sigma_delta_modulator #(
    parameter ORDER = 2   // Modulator order: 1 = first-order, 2 = second-order
) (
    ...
    // =========================================================================
    // Sigma-delta modulator: PCM → PDM
    // Runs at PDM rate (pdm_valid strobe, 3.125 MHz).
    // ORDER parameter selects 1st-order (20 dB/decade) or 2nd-order (40 dB/decade).
    // =========================================================================
    wire amp_dat_w;
