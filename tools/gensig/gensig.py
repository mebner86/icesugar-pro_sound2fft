#!/usr/bin/env python3
"""
gensig.py — Generate input CSV files for fpgactrl.

Writes a two-column CSV (time_s, speaker) that fpgactrl run -input can consume.

Usage:
  python3 gensig.py chirp  [options]
  python3 gensig.py sine   [options]
  python3 gensig.py noise  [options]

Common options (all signals):
  -o / --output FILE       Output CSV path          (default: sig_<signal>.csv)
...
def write_csv(path, fs, samples, meta=None):
...
def add_common_args(p):
    p.add_argument(
        ...
        "--output",
        default=None,
        help="Output CSV file (default: sig_<signal>.csv)",
    )
    p.add_argument(
        "-n",
        "--samples",
...
def main():
    ...
    output = args.output or f"sig_{args.signal}.csv"
    ...
    write_csv(output, args.fs, samples, meta)
